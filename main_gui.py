...
def add_doctor_gui(conn):
    ...
    def submit():
        ...
        success, message, temp_password = add_doctor(conn, name, dept_id, specialization, username)
        if success:
            full_message = f"{message}\n\nTemporary Password: {temp_password}\nPlease ask the doctor to change it immediately!"
            messagebox.showinfo("Success", full_message)
            add_window.destroy()
        else:
            messagebox.showerror("Error", message)
        ...
        if success:
            if result:
                for dept in result:
                    ...
                    text_area.insert(tk.END, f"Doctor Count: {dept.get('DoctorCount', 'N/A')}\n")
                    text_area.insert(tk.END, "-"*40 + "\n")
            else:
